	...
	<div class="section">
		...
		<div><ul class="select">
			<c:choose>
				...
				<c:otherwise><li><a href="javascript:sortPage('all', 'best', 1)">인기순</a></li></c:otherwise>
			</c:choose>
			<c:choose>
				<c:when test="${sort eq 'lowest'}"><li style="font-weight: bold;">낮은 가격순</li></c:when>
				<c:otherwise><li><a href="javascript:sortPage('all', 'lowest', 1)">낮은 가격순</a></li></c:otherwise>
			</c:choose>
			<c:choose>
				<c:when test="${sort eq 'highest'}"><li style="font-weight: bold;">높은 가격순</li></c:when>
				<c:otherwise><li><a href="javascript:sortPage('all', 'highest', 1)">높은 가격순</a></li></c:otherwise>
			</c:choose>
		</ul></div>
		<%-- 상품리스트 --%>
		<div><main>
			...
			<div id="pagination">
				<c:choose>
					<c:when test="${path eq '/product/all'}"><div><a href="javascript:page(${pd_list.navigateFirstPage}, 'all')">《</a></div></c:when>
					<c:otherwise><div><a href="javascript:sortPage('all', '${sort}', ${pd_list.navigateFirstPage})">《</a></div></c:otherwise>
				</c:choose>
				<c:forEach var="i" items="${pd_list.navigatepageNums}">
					<c:choose>
					<c:when test="${i==pd_list.pageNum}"><div class="selectpage">${i}</div></c:when>
					<c:when test="${path eq '/product/all'}"><div><a href="javascript:page(${i}, 'all')">${i}</a></div></c:when>
					<c:otherwise><div><a href="javascript:sortPage('all', '${sort}', ${i})">${i}</a></div></c:otherwise>
					</c:choose>
				</c:forEach>
				<c:choose>
					<c:when test="${path eq '/product/all'}"><div><a href="javascript:page(${pd_list.navigateLastPage}, 'all')">》</a></div></c:when>
					<c:otherwise><div><a href="javascript:sortPage('all', '${sort}', ${pd_list.navigateLastPage})">》</a></div></c:otherwise>
				</c:choose>
			</div>
		</main></div>
	</div>
